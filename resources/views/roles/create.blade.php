        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-5">
                    <form class="w-full" action="{{ route('role.store') }}" method="POST">
                        @csrf
                        <div class="rounded shadow py-3 px-4 w-full bg-white dark:bg-gray-800">
                            <div class="w-full px-3">
                                ...
                                <input name="name" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name1" type="text" placeholder="Role Name" required value="{{ old('name') }}" />
                            </div>
                            <div class="w-full px-3 mt-3">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                                    Description
                                </label>
                                <textarea name="description" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" rows="6" placeholder="Description">{{ old('description') }}</textarea>
                            </div>
                        </div>
                                        ...
                                        >
                                            ...
                                            <div class="px-4 py-2" x-ref="permissions">
                                                @foreach($permissions as $permission)
                                                    ...
                                                    <label class="block">
                                                        <input type="checkbox" id="permission-{{ $permission->id }}" name="permissions[]" value="{{ $permission->id }}" class="form-checkbox h-4 w-4 text-purple-600 rounded focus:outline-none focus:ring focus:border-blue-300"
                                                               x-on:click="
                                                                   let allChecked = true
