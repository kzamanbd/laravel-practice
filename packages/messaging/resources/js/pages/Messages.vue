<script setup lang="ts">
    ...
    if (selectedConversation.value) {
        form.value.conversation_id = selectedConversation.value.id;
    }
    ...
    const filteredUsers = computed(() => {
        return users.value.filter((d: User) => {
            return d.name.toLowerCase().includes(chat.value.searchUser.toLowerCase());
        });
    });
    ...
    const sendMessage = async () => {
        ...
        try {
            const { data: response } = await http.post('message', form.value);
