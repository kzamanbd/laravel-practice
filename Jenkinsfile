pipeline {
    ...
    post {
        success {
            ...
            sshagent(credentials: ['dev_api']) {
                sh 'ssh -o StrictHostKeyChecking=no root@203.188.245.58 unzip -o /home/ec2-user/artifact/artifact.zip -d /var/www/html'
                script {
                    try {
                        sh 'ssh -o StrictHostKeyChecking=no root@203.188.245.58 sudo chmod 777 /var/www/html/storage -R'
                    } catch (Exception e) {
